...
1. Choose the Starlight component you want to override.
   You can find a full list of components in the Overrides Reference.
   ...
   This example will override Starlight's `SocialIcons` component in the page nav bar.
...
3. Tell Starlight to use your custom component in the `components` configuration option in `astro.config.mjs`:
...
If you are reusing the `PageFrame` or `TwoColumnContent` components which contain [named slots](https://docs.astro.build/en/basics/astro-components/#named-slots), you also need to [transfer](https://docs.astro.build/en/basics/astro-components/#transferring-slots) these slots as well.
...
When overriding a Starlight component, you can access the global `starlightRoute` object containing all the data for the current page.
This allows you to use these values to control how your component template renders.

In the following example, a replacement `PageTitle` component displays the current page's title as set in the content's frontmatter:
...
Learn more about all the available properties in the Route Data Reference.
...
In the following example, a component overriding Starlight's `Footer` displays "Built with Starlight" on the homepage only, and otherwise shows the default footer on all other pages:
