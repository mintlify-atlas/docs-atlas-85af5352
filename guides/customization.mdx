...
This works particularly well for landing pages.
...
To enable edit links, set `editLink.baseUrl` to the URL used to edit your repository in the Starlight integration config.
...
If you already have font files, follow the local set-up guide.
To use Google Fonts, follow the Fontsource set-up guide.
...
3. Add Fontsource CSS files to Starlight's `customCss` array in `astro.config.mjs`:
   ...
   Fontsource ships multiple CSS files for each font. See the [Fontsource documentation](https://fontsource.org/docs/quickstartinstall#4-weights-and-styles) on including different weights and styles to understand which to use.
